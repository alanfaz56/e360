...
<div class="min-h-svh bg-sand-100">
	<main class="mx-auto max-w-3xl px-4 py-10">
		...
		<p class="mt-3 max-w-xl leading-relaxed text-sand-600">
			Estación 360 recibe la unidad, la diagnostica, la cotiza y responde ante el cliente. Cuando el trabajo es lo
			tuyo, te lo mandamos ya autorizado. Tú te concentras en repararlo bien.
		</p>
		...
		<section class="mt-8 rounded-lg border border-sand-200 bg-white p-5">
			...
			<p class="mt-4 rounded border border-sand-200 bg-sand-50 p-3 text-sm leading-relaxed text-sand-700">
				Cada solicitud la revisa una persona del equipo. Te llamamos para conocerlos y ver el taller antes de
				mandarles la primera unidad. <strong>No cobramos por registrarte ni por aparecer.</strong>
			</p>
		</section>
		...
		{#if form?.message}
			<p
				role="alert"
				class="mt-8 rounded border border-brand-300 bg-brand-50 px-3 py-2 text-sm text-brand-900"
			>
				{form.message}
			</p>
		{/if}
		...
		>
			<section class="rounded-lg border border-sand-200 bg-white p-5">
				...
				<div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
					...
					<Field
						...
						min="0"
						max="200"
						value={v("anosOperando")}
					/>
					<Field
						label="¿Cuántos trabajan ahí?"
						name="empleados"
						type="number"
						min="0"
						max="10000"
					...
					>
						{#snippet children(id)}
							<textarea
								...
								rows="2"
								class={INPUT}
								placeholder="Equipo con el que cuentan, marcas que dominan, horario…"
								>{v("notas")}</textarea
							>
						{/snippet}
					</Field>
			...
			<section class="rounded-lg border border-sand-200 bg-white p-5">
				...
				<p
					class="mt-4 flex gap-2 rounded border border-sand-200 bg-sand-50 p-3 text-xs leading-relaxed text-sand-600"
				>
					<Phone
						size={14}
						aria-hidden="true"
			...
			<Button
				full
				size="lg">Enviar solicitud</Button
			>
