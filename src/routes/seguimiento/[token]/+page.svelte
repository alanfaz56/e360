	...
	<main class="mx-auto max-w-2xl space-y-4 px-4 py-6">
		<section class="rounded-lg border border-sand-200 bg-white p-5">
			...
			<h1 class="font-display mt-1 flex items-center gap-2 text-2xl text-sand-950">
				<Car size={22} aria-hidden="true" class="shrink-0 text-brand-600" />
		...
		<section class="rounded-lg border border-brand-200 bg-brand-50 p-4">
			...
			<p class="mb-3 mt-1 text-xs leading-relaxed text-sand-700">
				Te mandamos un aviso a este teléfono cuando tu unidad avance o esté lista. Puedes apagarlo
				cuando quieras.
			</p>
			<PushToggle clavePublica={data.clavePublica} endpoint={`/api/seguimiento/${data.token}`} />
		</section>
			...
			<section class="rounded-lg border border-sand-200 bg-white p-5">
				...
				<ul class="mt-3 space-y-2 text-sm">
					{#each data.cotizaciones as c (c.folio)}
						<li class="flex flex-wrap items-center gap-2 border-b border-sand-100 pb-2">
							<span class="font-medium text-sand-900">Cotización #{c.folio}</span>
							<span class="text-sand-600">${c.total}</span>
							<span class="ml-auto text-xs text-sand-500">{c.estado}</span>
						</li>
					{/each}
					{#each data.facturas as f (f.folio)}
						<li class="flex flex-wrap items-center gap-2 border-b border-sand-100 pb-2 last:border-0">
							<span class="font-medium text-sand-900">Factura #{f.folio}</span>
							<span class="text-sand-600">${f.total}</span>
							<span class="ml-auto text-xs text-sand-500">
								{f.estado}{f.vence ? ` · vence ${f.vence.slice(0, 10)}` : ""}
							</span>
						</li>
					{/each}
				</ul>
