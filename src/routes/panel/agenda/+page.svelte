...
<PageHeader
	title="Agenda"
	description="Lo que entra al taller esta semana."
>
	{#snippet actions()}
		<Button
			href="/panel/citas"
...
{#if form?.creada}
	<!-- This action does not redirect (the drawer keeps its form state on failure), so its own
	     success flag stands in for the `?ok=` a redirect would have carried. -->
	<p
		role="status"
		class="mb-4 rounded border border-ok bg-ok/15 px-3 py-2 text-sm text-sand-800"
	>
		Cita agendada.
	</p>
{:else}
...
{#if data.mias}
	<p
		class="mb-3 flex flex-wrap items-center gap-2 rounded border border-brand-200 bg-brand-50 px-3 py-2 text-sm text-brand-900"
	>
		Mostrando solo las citas asignadas a ti.
		<a
			class="font-medium underline"
			href={searchHref(page.url, { mias: null })}>Ver todas</a
		>
	</p>
{/if}
