...
<div class="max-w-5xl mx-auto">
	<PageHeader
		...
		description="La situación del taller: dinero, operación y problemas, primero."
	/>

	<!-- Real GET form: los filtros SON la URL, así cualquier vista es compartible y funciona sin JS. -->
	<form
		method="GET"
		class="mb-6 grid gap-3 rounded-lg border border-sand-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-5"
	>
		<Field
			label="Rango"
			name="rango"
			...
			{#snippet children(id)}
				...
				>
					{#each data.rangoOpciones as r (r.value)}
						<option
							value={r.value}
							selected={data.periodo.rango === r.value}>{r.label}</option
						>
					{/each}
				</select>
			{/snippet}
			...
			{#snippet children(id)}
				...
				>
					...
					{#each data.talleresOpciones as t (t.id)}
						<option
							value={t.id}
							selected={data.filtros.taller === t.id}>{t.nombre}</option
						>
					{/each}
				</select>
			{/snippet}
			...
			{#snippet children(id)}
				...
				>
					...
					{#each data.mecanicosOpciones as m (m.id)}
						<option
							value={m.id}
							selected={data.filtros.mecanico === m.id}>{m.name}</option
						>
					{/each}
				</select>
			{/snippet}
