...
<PageHeader
	title="Clientes"
	description="Personas y organizaciones que traen unidades al taller."
>
	{#snippet actions()}
		{#if data.puede.crear}
			...
			{#snippet children(id)}
				...
				>
					...
					{#each data.tipos as t (t.value)}
						<option
							value={t.value}
							selected={t.value === data.filtros.tipo}>{t.label}</option
						>
					{/each}
				</select>
			{/snippet}
...
{:else}
	<div class="mt-6">
		<DataTable
			columns={["Nombre", "Tipo", "Teléfono", "Unidades", "Contactos", ""]}
			items={data.clientes}
		>
			{#snippet row(cliente)}
