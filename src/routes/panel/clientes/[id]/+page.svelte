<script lang="ts">
	...
	const alcance = $derived(
		alcanceElegido ?? (editando?.alcanceUnidades as "todas" | "especificas") ?? "todas",
	);
...
<a href="/panel/clientes" class="mb-4 inline-flex items-center gap-1.5 text-sm text-sand-600 hover:text-brand-700">
	<ArrowLeft size={16} aria-hidden="true" />
	Clientes
</a>

<PageHeader title={data.cliente.nombreCompleto} description={data.cliente.tipoLabel}>
	{#snippet actions()}
		...
		{#if data.puede.archivar}
			<form method="POST" action="?/archivar">
				<input type="hidden" name="archivado" value={data.cliente.archivado ? "false" : "true"} />
...
<section class="mt-6 rounded-lg border border-sand-200 bg-white p-5">
	...
	<dl class="mt-3 grid gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
		<div><dt class="text-sand-500">Teléfono</dt><dd class="text-sand-950">{data.cliente.telefono ?? "—"}</dd></div>
		<div><dt class="text-sand-500">Correo</dt><dd class="text-sand-950">{data.cliente.email ?? "—"}</dd></div>
		<div class="sm:col-span-2"><dt class="text-sand-500">Dirección</dt><dd class="text-sand-950">{data.cliente.direccion ?? "—"}</dd></div>
		<div><dt class="text-sand-500">RFC</dt><dd class="text-sand-950">{data.cliente.rfc ?? "—"}</dd></div>
		<div><dt class="text-sand-500">Régimen fiscal</dt><dd class="text-sand-950">{satLabel(REGIMENES_FISCALES, data.cliente.regimenFiscal) ?? "—"}</dd></div>
		<div><dt class="text-sand-500">Código postal</dt><dd class="text-sand-950">{data.cliente.codigoPostal ?? "—"}</dd></div>
		<div><dt class="text-sand-500">Uso CFDI</dt><dd class="text-sand-950">{satLabel(USOS_CFDI, data.cliente.usoCfdi) ?? "—"}</dd></div>
		{#if data.cliente.notas}
			<div class="sm:col-span-2"><dt class="text-sand-500">Notas</dt><dd class="text-sand-950">{data.cliente.notas}</dd></div>
		{/if}
	</dl>
</section>
...
<div class="mt-10 flex flex-wrap items-center gap-3">
	...
	{#if data.puede.contactos}
		<Button href={searchHref(page.url, { drawer: "contacto" })} size="sm" class="ml-auto">
...
{#if data.cliente.tipo === "persona"}
	<p class="mt-1 text-sm text-sand-600">
		{data.cliente.nombreCompleto} puede recoger sus propias unidades sin estar en esta lista. Agrega
		aquí a quien más pueda hacerlo.
	</p>
{:else}
	<p class="mt-1 text-sm text-sand-600">
...
{#if data.contactos.length === 0}
	<p class="mt-3 rounded-lg border border-dashed border-sand-300 bg-white px-4 py-6 text-center text-sm text-sand-600">
		Sin personas registradas.
	</p>
{:else}
	<div class="mt-3">
		<DataTable columns={["Nombre", "Roles", "Contacto", "Puede recoger", ""]} items={data.contactos}>
				...
				<td class="px-4 py-2.5 text-right">
					{#if data.puede.contactos}
						<div class="flex justify-end gap-1">
							...
							<form method="POST" action="?/eliminarContacto">
...
<div class="mt-10 flex flex-wrap items-center gap-3">
	...
	{#if data.puede.crearUnidad && !data.cliente.archivado}
		<Button href={searchHref(page.url, { drawer: "unidad" })} size="sm" class="ml-auto">
...
{:else}
	<div class="mt-3">
		<DataTable columns={["Unidad", "Placas", "VIN", "Económico", "Km", ""]} items={data.unidades}>
			{#snippet row(unidad)}
				<td class="px-4 py-2.5 font-medium text-sand-950">
...
{#if drawer === "editar" && data.puede.editar}
	<Drawer title="Editar cliente" description={data.cliente.nombreCompleto} closeHref={closeDrawer}>
		<form method="POST" action="?/editar" class="space-y-4">
			...
			{#if tipo === "persona"}
				<Field label="Nombre" name="nombre" required value={data.cliente.nombre ?? ""} />
				<Field label="Apellidos" name="apellidos" value={data.cliente.apellidos ?? ""} />
			{:else}
				<Field label="Razón social" name="razonSocial" required value={data.cliente.razonSocial ?? ""} />
			{/if}
			<Field label="Teléfono" name="telefono" type="tel" value={data.cliente.telefono ?? ""} />
			<Field label="Correo" name="email" type="email" value={data.cliente.email ?? ""} />
			<Field label="Dirección" name="direccion" value={data.cliente.direccion ?? ""} />
			<Field label="RFC" name="rfc" value={data.cliente.rfc ?? ""} />
			<SatSelect
			...
			/>
			<Field label="Código postal" name="codigoPostal" value={data.cliente.codigoPostal ?? ""} />
			<SatSelect label="Uso CFDI" name="usoCfdi" catalogo={USOS_CFDI} {tipo} value={data.cliente.usoCfdi} />
			<Field label="Notas" name="notas" value={data.cliente.notas ?? ""} />
...
{#if drawer === "contacto" && data.puede.contactos}
	...
	>
		<form method="POST" action={editando ? "?/editarContacto" : "?/crearContacto"} class="space-y-4">
			{#if editando}
				<input type="hidden" name="contactoId" value={editando.id} />
			{/if}

			<Field label="Nombre" name="nombre" required value={editando?.nombre ?? ""} />
			<Field label="Teléfono" name="telefono" type="tel" value={editando?.telefono ?? ""} />
			<Field label="Correo" name="email" type="email" value={editando?.email ?? ""} />
			<Field
			...
			<fieldset>
					...
					{#each data.rolesDisponibles as rol (rol.value)}
						<label class="flex items-start gap-2.5 text-sm">
							...
							<span>
								...
								{#if rol.autoridad}
									<ShieldCheck size={13} class="ml-1 inline text-brand-600" aria-label="Rol con autoridad" />
								{/if}
				...
				{#if !data.puede.otorgarAutoridad}
					<p class="mt-2 rounded border border-accent-500/60 bg-accent-500/15 px-3 py-2 text-xs text-sand-800">
			...
			<Field label="Puede recoger" name="alcanceUnidades">
				{#snippet children(id)}
					<select
						{id}
			...
			<Field label="Notas" name="notas" value={editando?.notas ?? ""} />
...
{#if drawer === "unidad" && data.puede.crearUnidad}
	<Drawer
		...
		closeHref={closeDrawer}
	>
		<form method="POST" action="?/crearUnidad" class="space-y-4">
			<Field label="Marca" name="marca" required />
			<Field label="Modelo" name="modelo" required />
			<Field label="Año" name="anio" type="number" />
			<Field label="Color" name="color" />
			<Field label="Placas" name="placas" />
			<Field label="VIN / NIV" name="vin" hint="Único en todo el sistema cuando se captura." />
			<Field label="Número económico" name="numeroEconomico" hint="El número que la flotilla pinta en la puerta." />
			<Field label="Kilometraje" name="kilometraje" type="number" />
